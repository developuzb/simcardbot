"""
Texnoset — Sotuv agenti CHAT BACKEND (sayt uchun)
=================================================
...
ISHGA TUSHIRISH (3 qadam):
  ...
  2) AI kalitini env'ga bering (Groq BEPUL va tez — pastdagi README'ga qarang):
       Windows:  set LLM_API_KEY=gsk_xxxxx
       Linux/Mac: export LLM_API_KEY=gsk_xxxxx
  3) Serverni ishga tushiring:
       python chat_backend.py
  So'ng saytdagi  CHAT_API  ga shu server manzilini yozing
  (masalan http://localhost:8000/chat  yoki joylangan domeningiz).
...
SOZLAMALAR (env, ixtiyoriy):
  ...
  LLM_MODEL     — default: llama-3.3-70b-versatile (Groq)
                  OpenAI uchun masalan: gpt-4o-mini
  PORT          — default: 8000
  ALLOW_ORIGIN  — default: *  (sayt domeningizni qo'ysangiz xavfsizroq)
"""
import os
import time
import logging
...
def chat():
    ...
    if not isinstance(history, list):
        history = []
    # faqat ruxsat etilgan maydonlar, oxirgi MAX_HISTORY ta
    clean = [
